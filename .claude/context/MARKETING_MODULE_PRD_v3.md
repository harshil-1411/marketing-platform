...
> **v3.0 Change Note:** Corrected Meta WhatsApp API token architecture. The original PRD incorrectly specified a per-tenant `access_token_secret_arn`. This has been replaced with the correct model: one platform-level Meta System User permanent token stored centrally in SSM/Secrets Manager, with tenants differentiated exclusively by their `phone_number_id` and `waba_id`. See Section 3.1.1, Section 3.5, Section 12.2, and Appendix B for full details.
...
Tenant:
  ...
  whatsapp_config:
    phone_number_id: string        # Tenant's unique Meta phone number ID — used in all API call URLs
    waba_id: string                # Tenant's unique WhatsApp Business Account ID — used for template management
    display_phone_number: string   # Human-readable number (e.g., "+91 98765 43210") for display only
    business_profile_verified: boolean
    # NOTE: No per-tenant access token is stored here.
    # All Meta Cloud API calls use the single platform-level System User token
    # stored centrally in SSM/Secrets Manager (see Section 3.5 and Appendix B).
    # Tenant isolation is achieved exclusively through phone_number_id and waba_id.
  loyalty_integration:
    enabled: boolean
...
### 3.5 Meta WhatsApp API Token Architecture

#### 3.5.1 One App, One Token, Many WABAs

The platform operates under a single Meta App (the "uni app" / platform Meta App). All tenant WhatsApp Business Accounts (WABAs) are registered and connected under this one Meta App.

**The token model is as follows:**

| Concept | Scope | How Many | Stored Where |
|---------|-------|----------|-------------|
| Meta System User permanent token | Platform-level (one for all tenants) | **One** | SSM Parameter Store (Phase 1) / Secrets Manager (Phase 2+) |
| `phone_number_id` | Per tenant | One per tenant | DynamoDB tenant record |
| `waba_id` | Per tenant | One per tenant | DynamoDB tenant record |

The System User permanent token is generated once in the Meta App dashboard under **System Users** and never changes unless manually rotated by the platform operator. It is **not** generated per tenant, per WABA, or per phone number. It is a single app-level credential.

#### 3.5.2 How Tenant Isolation Works at the API Call Level

Even though all tenants share the same token, every Meta Cloud API call is scoped to a specific tenant through the URL path:

```
# Sending a message for Tenant A
POST /v21.0/{tenant_A_phone_number_id}/messages
Authorization: Bearer {platform_system_user_token}

# Sending a message for Tenant B
POST /v21.0/{tenant_B_phone_number_id}/messages
Authorization: Bearer {platform_system_user_token}

# Managing templates for Tenant A
POST /v21.0/{tenant_A_waba_id}/message_templates
Authorization: Bearer {platform_system_user_token}

# Managing templates for Tenant B
POST /v21.0/{tenant_B_waba_id}/message_templates
Authorization: Bearer {platform_system_user_token}
```

The token authenticates the platform. The `phone_number_id` and `waba_id` tell Meta which business account the action applies to. Meta enforces that the System User token has been granted access to each WABA (this happens automatically during Embedded Signup onboarding).

#### 3.5.3 How the Token Reaches Lambdas at Runtime
...
def get_meta_token() -> str:
    """
    Fetches the platform-level Meta System User token from SSM/Secrets Manager.
    Called once per Lambda cold start and cached in memory for the Lambda lifetime.
    Never stored in DynamoDB. Never passed to tenants.
    """
    backend = os.environ.get("SECRETS_BACKEND", "ssm")
    if backend == "secretsmanager":
        client = boto3.client("secretsmanager")
        return client.get_secret_value(
            SecretId=os.environ["META_SYSTEM_USER_TOKEN_SECRET_ARN"]
        )["SecretString"]
    else:
        client = boto3.client("ssm")
        return client.get_parameter(
            Name=os.environ["META_SYSTEM_USER_TOKEN_PARAM"],
            WithDecryption=True
        )["Parameter"]["Value"]


def send_whatsapp_template(
    phone_number_id: str,   # From tenant's DynamoDB record
    recipient_phone: str,
    template_name: str,
    language: str,
    components: list,
    token: str              # Platform token fetched once by Lambda handler
) -> dict:
    """
    phone_number_id differentiates which tenant's WhatsApp number is sending.
    token is the same for every tenant.
    """
...
#### 3.5.4 Tenant Onboarding — How a New WABA Gets Connected

When a new tenant completes the WhatsApp setup step (Section 10.1, Step 3):

1. Tenant clicks "Connect WhatsApp" in your onboarding wizard.
2. Meta's **Embedded Signup** popup opens inside your frontend (hosted by Meta, embedded via Meta's JavaScript SDK).
3. Tenant logs in with their Facebook/Meta Business account inside the popup.
4. Tenant verifies their phone number and grants your Meta App permission to manage their WABA.
5. Meta returns a **short-lived user token** to your frontend callback.
6. Your backend exchanges this short-lived token for the permanent association — Meta grants your platform's System User access to the new WABA.
7. Your backend calls Meta's API to retrieve the new `phone_number_id` and `waba_id`.
8. These two values are saved to the tenant's DynamoDB record under `whatsapp_config`.
9. The tenant's WABA is now fully accessible to your platform via the shared System User token.

**No new token is created for the tenant.** The existing platform System User token automatically gains access to the newly connected WABA.

#### 3.5.5 Implications for the Codebase

- The `whatsapp_config` block on the Tenant entity contains **only** `phone_number_id`, `waba_id`, `display_phone_number`, and `business_profile_verified`.
- There is **no** `access_token_secret_arn` on tenant records.
- The Meta System User token is a **platform secret**, not a tenant secret. It lives in one SSM parameter (`META_SYSTEM_USER_TOKEN_PARAM`) or one Secrets Manager secret (`META_SYSTEM_USER_TOKEN_SECRET_ARN`).
- All Lambda functions that call Meta APIs fetch this token once on cold start and reuse it.
- If the token ever needs to be rotated, it is regenerated once in the Meta dashboard, updated in SSM/Secrets Manager, and all Lambdas pick it up on their next cold start.
...
- Secrets: **Phase 1:** SSM Parameter Store SecureString (free, encrypted with AWS-managed KMS key) for the Meta System User token (platform-level, one parameter), payment gateway keys, and Loyalty platform API keys. Manual rotation. **Phase 2+:** AWS Secrets Manager with auto-rotation enabled. Migration is a config change — update environment variables to read from Secrets Manager ARN instead of SSM parameter name.

> **Important:** The Meta System User token is a single platform-level secret, not a per-tenant secret. It is stored once in SSM/Secrets Manager and shared across all Lambda functions that call Meta APIs. Tenant isolation is enforced at the API call level via `phone_number_id` and `waba_id` (see Section 3.5).
...
| `META_SYSTEM_USER_TOKEN_PARAM` | Platform-level Meta System User permanent token (SSM param name) | SSM (Phase 1) |
| `META_SYSTEM_USER_TOKEN_SECRET_ARN` | Platform-level Meta System User permanent token (Secrets Manager ARN) | Secrets Manager (Phase 2+) |
| `WHATSAPP_VERIFY_TOKEN` | Webhook verification challenge token (set once in Meta App dashboard) | SSM (Phase 1) / Secrets Manager (Phase 2+) |
...
| WABA | WhatsApp Business Account — a Meta-level account created per tenant during Embedded Signup. Each tenant has their own WABA with a unique `waba_id` and `phone_number_id` |
| System User Token | A permanent, app-level access token generated once in the Meta App dashboard under System Users. This is a platform-level credential shared across all tenants. Tenants are differentiated by their `phone_number_id` and `waba_id`, not by separate tokens |
| phone_number_id | The unique Meta identifier for a tenant's specific WhatsApp phone number. Used in the URL path of all Meta API calls for sending messages and media |
| waba_id | The unique Meta identifier for a tenant's WhatsApp Business Account. Used in the URL path of all Meta API calls for template management |
...
| Embedded Signup | Meta's official guided onboarding flow embedded in your frontend. Used to create and connect a tenant's WABA to the platform Meta App without requiring the tenant to manually generate tokens |
...
*v3.0: Corrected Meta WhatsApp API token architecture — platform-level System User token, tenant isolation via phone_number_id and waba_id.*
